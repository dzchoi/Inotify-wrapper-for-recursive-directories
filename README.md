...
The library is just a single file `inotify.hpp`. We just need to #include it and compile it. (`syslog.hpp` is optional, and will provide a function object wrapping `syslog()` system call.)

```
$ g++ g++ -O2 test.cpp -lstdc++fs
```
